function startDjikstra(s_node, e_node, n_map){
    var dijkstra= {
        findPath:function(start,end,rows,cols,grid)
        {
            var openList=[];
            openList.push(start);
            var node_loop = setInterval(function(){
                ...
                // Grab the lowest f(x) to process next(for dijkstra's f(x)=g(x) as h(x)=0)
                var lowInd=0;
                ...
                if (grid[currentNode[0]][currentNode[1]][3]=="Goal") 
                    {
                ...
                var neighbors=dijkstra.neighbors(grid,currentNode,rows,cols);
                for(var i=0;i<neighbors.length;i++)
                    {
                        ...
                        var gscore=grid[currentNode[0]][currentNode[1]][0]+1;   // 1 is the distance from a node                                                               to it's neighbor
        ...
        neighbors: function(grid,node,rows,cols)
        {
            var ret=[];
            var x=node[0];
            var y=node[1];
    ...
    function dijkstraPathFinder(row_count,col_count,startCoordinates,goalCoordinates,obstacleList)
    {
        var rows=row_count;
        ...
        for(var i=0;i<rows;i++)
            {
                grid[i]=[];
                for(var j=0;j<cols;j++)     //the format for each node is [g,h,f,empty/obstacle,parent,closed,visited]
                    {
        ...
        var finalpath=dijkstra.findPath([s_node[1], s_node[0]],[e_node[1], e_node[0]],rows,cols,grid);
        return finalpath;
    }
    ...
    var finalpath= dijkstraPathFinder(row_grid, col_grid, [s_node[1], s_node[0]], [e_node[1], e_node[0]],obstacles);
    //printing the path
